...
gulp.task('installComponentDependencies', ['installMainDependencies'], function() {
    ...
    .pipe(shell([
      ...
      templateData: {
        f: function (file, base) {
          var p = /^.*\/bower_components\/(.*)\/package[.]json/.exec(file.path)[1];
          return file.base + p;
        }
      }
    }));
...
function compileStyles(file, cb) {
  ...
  if (file.path.match(/bower_components/)) {
    let p = /^.*\/(bower_components\/.*)\/component/.exec(file.base)[1] + '/resources/less';
    lessPaths.push(p);
  }
...
function fixComponentBasePath(file, cb){
  if(file.path.match(/bower_components/)) {
...
gulp.task('copyComponentViewFiles', ['clean', 'copyMainBuildFiles',
  'compileBaseStyle', 'installDependencies'], function() {
    return gulp.src(['src/main/component/**/View.html', 'bower_components/*/component/**/View.html'])
...
gulp.task('copyControllerFiles', ['clean', 'copyMainBuildFiles',
  'compileBaseStyle', 'installDependencies'], function() {
    return gulp.src(['src/main/component/**/*.js', 'bower_components/*/component/**/*.js'])
      .pipe(map(fixComponentBasePath))
      .pipe(gulp.dest('build/frontend/component'));
  });

gulp.task('compileBaseStyle', ['copyMainBuildFiles', 'generateVariablesLessFile'], function() {
...
gulp.task('installDependencies', ['installMainDependencies', 'installComponentDependencies']);
